...
class NarrativeService {
    ...
    transformAIResult(aiNarrative, analyticsData, tone) {
        const { metrics, insights, forecasts, recommendations } = analyticsData;
    ...
    buildNarrativeContext(metrics, insights, forecasts, recommendations, labels) {
        ...
        const amountMetrics = Object.entries(metrics).find(([col, metric]) => 
            col.toLowerCase().includes('amount') && metric.average
        );
        if (amountMetrics) {
        ...
        if (forecasts.revenue && forecasts.revenue.nextMonth.confidence) {
            const confidence = forecasts.revenue.nextMonth.confidence;
    ...
    generateOperationalNarrative(metrics, insights, recommendations, tone) {
        ...
        const operationalRecs = recommendations.filter(r => 
            r.title.toLowerCase().includes('operational') || 
