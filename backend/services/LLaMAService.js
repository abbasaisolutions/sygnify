...
class LLaMAService {
    constructor(config = {}) {
        this.config = {
            ...
            fallbackService: config.fallbackService || null,
...
6. Model Performance Metrics (specific performance indicators)`
    ...
    async generateNarrative(analyticsData, style = 'executive', options = {}) {
        try {
            ...
            // Fallback to template-based generation if enabled
            if (this.config.enableFallback && this.config.fallbackService) {
                console.log('Falling back to template-based generation');
                return await this.fallbackGeneration(analyticsData, style, options);
            }
            
            return {
                success: false,
                error: error.message,
                narrative: this.generateErrorNarrative(style),
                metadata: {
                    style: style,
                    error: true,
                    generatedAt: new Date().toISOString()
    ...
    generateHeadline(data) {
        const recordCount = data.summary.recordCount;
    ...
    calculateLiquidityScore(metrics) {
        ...
        } else if (balance > 0 && Math.abs(amount) < balance * 0.2) {
