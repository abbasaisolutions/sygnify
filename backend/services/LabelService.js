  ...
  loadGlossary() {
    try {
      ...
      const possiblePaths = [
        ...
        path.join(process.cwd(), 'backend/glossary.json'),
      ];

      for (const glossaryPath of possiblePaths) {
        if (fs.existsSync(glossaryPath)) {
          const glossaryData = fs.readFileSync(glossaryPath, 'utf8');
          this.glossary = JSON.parse(glossaryData);
          console.log(`✅ Loaded glossary from: ${glossaryPath}`);
          return;
        }
      }

      // If no glossary file found, use comprehensive fallback
      console.log('⚠️ No glossary file found, using comprehensive fallback glossary');
      this.glossary = this.getComprehensiveFallbackGlossary();
    } catch (error) {
      console.warn('⚠️ Could not load glossary, using fallback:', error.message);
      this.glossary = this.getComprehensiveFallbackGlossary();
    }
  }

  getComprehensiveFallbackGlossary() {
    return {
      finance: {
        // Transaction-related
        transaction_id: {
          label: 'Transaction ID', category: 'Identifier', type: 'categorical', importance: 90,
        },
        transaction_date: {
          label: 'Transaction Date', category: 'Temporal', type: 'date', importance: 95,
        },
        amount: {
          label: 'Transaction Amount', category: 'Monetary', type: 'numeric', importance: 100,
        },
        currency: {
          label: 'Currency', category: 'Monetary', type: 'categorical', importance: 80,
        },

        // Account-related
        account_id: {
          label: 'Account ID', category: 'Identifier', type: 'categorical', importance: 85,
        },
        account_type: {
          label: 'Account Type', category: 'Classification', type: 'categorical', importance: 75,
        },
        balance: {
          label: 'Account Balance', category: 'Monetary', type: 'numeric', importance: 90,
        },

        // Customer-related
        customer_id: {
          label: 'Customer ID', category: 'Identifier', type: 'categorical', importance: 85,
        },
        customer_name: {
          label: 'Customer Name', category: 'Personal', type: 'categorical', importance: 70,
        },
        customer_type: {
          label: 'Customer Type', category: 'Classification', type: 'categorical', importance: 75,
        },

        // Fraud-related
        is_fraud: {
          label: 'Fraud Flag', category: 'Risk', type: 'boolean', importance: 95,
        },
        fraud_score: {
          label: 'Fraud Score', category: 'Risk', type: 'numeric', importance: 90,
        },
        risk_level: {
          label: 'Risk Level', category: 'Risk', type: 'categorical', importance: 85,
        },

        // Merchant-related
        merchant_id: {
          label: 'Merchant ID', category: 'Identifier', type: 'categorical', importance: 80,
        },
        merchant_name: {
          label: 'Merchant Name', category: 'Business', type: 'categorical', importance: 75,
        },
        merchant_category: {
          label: 'Merchant Category', category: 'Classification', type: 'categorical', importance: 80,
        },
        merchant_state: {
          label: 'Merchant State', category: 'Location', type: 'categorical', importance: 70,
        },

        // Transaction details
        transaction_type: {
          label: 'Transaction Type', category: 'Classification', type: 'categorical', importance: 85,
        },
        payment_method: {
          label: 'Payment Method', category: 'Classification', type: 'categorical', importance: 80,
        },
        card_type: {
          label: 'Card Type', category: 'Classification', type: 'categorical', importance: 75,
        },

        // Financial metrics
        revenue: {
          label: 'Revenue', category: 'Monetary', type: 'numeric', importance: 95,
        },
        profit: {
          label: 'Profit', category: 'Monetary', type: 'numeric', importance: 90,
        },
        cost: {
          label: 'Cost', category: 'Monetary', type: 'numeric', importance: 85,
        },
        sales: {
          label: 'Sales', category: 'Monetary', type: 'numeric', importance: 90,
        },
        expenses: {
          label: 'Expenses', category: 'Monetary', type: 'numeric', importance: 85,
        },

        // Temporal
        date: {
          label: 'Date', category: 'Temporal', type: 'date', importance: 85,
        },
        month: {
          label: 'Month', category: 'Temporal', type: 'categorical', importance: 75,
        },
        year: {
          label: 'Year', category: 'Temporal', type: 'numeric', importance: 80,
        },
        quarter: {
          label: 'Quarter', category: 'Temporal', type: 'categorical', importance: 70,
        },

        // Location
        country: {
          label: 'Country', category: 'Location', type: 'categorical', importance: 75,
        },
        state: {
          label: 'State', category: 'Location', type: 'categorical', importance: 70,
        },
        city: {
          label: 'City', category: 'Location', type: 'categorical', importance: 65,
        },
        zip_code: {
          label: 'Zip Code', category: 'Location', type: 'categorical', importance: 60,
        },
      },
      ...
      general: {
        // Generic patterns
        id: {
          label: 'ID', category: 'Identifier', type: 'categorical', importance: 80,
        },
        name: {
          label: 'Name', category: 'Personal', type: 'categorical', importance: 75,
        },
        type: {
          label: 'Type', category: 'Classification', type: 'categorical', importance: 70,
        },
        category: {
          label: 'Category', category: 'Classification', type: 'categorical', importance: 75,
        },
        status: {
          label: 'Status', category: 'Classification', type: 'categorical', importance: 80,
        },
        created_at: {
          label: 'Created At', category: 'Temporal', type: 'date', importance: 70,
        },
        updated_at: {
          label: 'Updated At', category: 'Temporal', type: 'date', importance: 65,
        },
      },
    };
  }
  ...
  async extractLabels(data, userId = null) {
    ...
    for (const column of columns) {
      ...
      labels[column] = {
        semantic,
        category,
        importance, // Normalized 0-100 scale
        type: dataType,
