...
class PredictionService {
    ...
    extractTimeSeriesData(data, columnName, labels) {
        ...
        const dateColumn = Object.keys(labels).find(col => 
            labels[col].semantic === 'Transaction Date' || 
    ...
    calculateAccuracy(actual, predicted) {
        const mape = actual.reduce((sum, val, i) => 
            sum + Math.abs((val - predicted[i]) / val), 0) / actual.length;
    ...
    calculateCorrelation(x, y) {
        ...
        const numerator = xSlice.reduce((sum, xi, i) => 
            sum + (xi - xMean) * (ySlice[i] - yMean), 0);
        
        const xDenom = Math.sqrt(xSlice.reduce((sum, xi) => 
            sum + Math.pow(xi - xMean, 2), 0));
        const yDenom = Math.sqrt(ySlice.reduce((sum, yi) => 
