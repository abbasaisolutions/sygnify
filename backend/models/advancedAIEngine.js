...
class AdvancedAIEngine {
  ...
  async forecastShortTerm(processedData) {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      forecast.push({
        month: i,
        revenue: baseRevenue * (1 + growthRate) ** i,
        confidence: 'medium',
      });
    }
  ...
  findRevenueColumns(data) {
    ...
    return columns.filter((col) => col.toLowerCase().includes('revenue')
  ...
  findExpenseColumns(data) {
    const columns = Object.keys(data[0] || {});
    return columns.filter((col) => col.toLowerCase().includes('expense')
  ...
  extractAmount(row) {
    const amountColumns = Object.keys(row).filter((col) => col.toLowerCase().includes('amount')
  ...
  extractDate(row) {
    const dateColumns = Object.keys(row).filter((col) => col.toLowerCase().includes('date')
  ...
  identifyRevenueSource(row) {
    const sourceColumns = Object.keys(row).filter((col) => col.toLowerCase().includes('source')
  ...
  categorizeExpense(row) {
    const categoryColumns = Object.keys(row).filter((col) => col.toLowerCase().includes('category')
      || col.toLowerCase().includes('type')
  ...
  detectSeasonality(values) {
    ...
    const variance = Object.values(monthlyAverages).reduce((sum, val) => sum + (val - avg) ** 2, 0) / 12;
