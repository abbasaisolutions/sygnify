...
async function testImportanceScoring() {
    ...
    try {
        ...
        if (allValid) {
            ...
            console.log('📊 Sample importance scores:', 
                Object.entries(labels).slice(0, 3).map(([col, label]) => 
                    `${col}: ${label.importance}`
                ).join(', ')
            );
        } else {
...
async function testSemanticLabeling() {
    ...
    try {
        ...
        const binaryFields = Object.entries(labels).filter(([col, label]) => 
            label.type === 'binary' || col.includes('is_') || col.includes('flag_')
        );
...
async function testDynamicNarrative() {
    ...
    try {
        ...
        const hasMetrics = narratives.some(n => 
            n.main.summary.includes('10') || n.main.summary.includes('record')
        );
